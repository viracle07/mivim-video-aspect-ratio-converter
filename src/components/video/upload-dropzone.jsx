...
export function UploadDropzone() {
  ...
  const [backgroundColor, setBackgroundColor] = useState("#101418");
  ...
  async function startUpload() {
    ...
    try {
      const response = await fetch("/api/convert", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: file.name, ratio, fitMode, quality, frameRate, backgroundColor, sizeBytes: file.size, ...metadata }) });
    ...
    <div className="rounded-lg border border-line bg-white p-5">
      ...
      {file && <div className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(18rem,0.75fr)]">
        ...
        <div className="space-y-5">
          ...
          {fitMode === "solid" && <label className="flex items-center justify-between rounded-md border border-line px-3 py-2 text-sm font-medium">Canvas colour<input type="color" value={backgroundColor} onChange={(event) => setBackgroundColor(event.target.value)} disabled={state === "uploading" || state === "complete"} className="h-8 w-10 cursor-pointer border-0 bg-transparent p-0" aria-label="Canvas colour" /></label>}
          <div className="grid grid-cols-2 gap-3"><label className="text-sm font-medium">Output quality<select value={quality} onChange={(event) => setQuality(event.target.value)} disabled={state === "uploading" || state === "complete"} className="mt-2 h-10 w-full rounded-md border border-line bg-white px-3"><option value="720p">720p - Faster</option><option value="1080p">1080p - Higher quality</option></select></label><label className="text-sm font-medium">Frame rate<select value={frameRate} onChange={(event) => setFrameRate(event.target.value)} disabled={state === "uploading" || state === "complete"} className="mt-2 h-10 w-full rounded-md border border-line bg-white px-3"><option value="original">Keep original</option><option value="24">24 fps</option><option value="30">30 fps</option><option value="60">60 fps</option></select></label></div>
