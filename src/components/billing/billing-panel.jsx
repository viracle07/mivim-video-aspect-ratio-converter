...
export function BillingPanel() {
  ...
  async function manageSubscription(action) {
    if (action === "disable" && !window.confirm("Cancel automatic renewal? Your access will remain active until the current period ends.")) return;
    setBusyPlan(action); setMessage("");
    try {
      const response = await fetch("/api/paystack/subscription", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Subscription could not be updated.");
      setMessage(action === "enable" ? "Automatic renewal restored." : "Automatic renewal cancelled.");
      window.location.reload();
    } catch (error) { setMessage(error.message); setBusyPlan(""); }
  }
  ...
  return <div className="space-y-6">
    ...
    {active && <Card><CardContent className="flex flex-wrap items-center justify-between gap-4"><div><p className="font-semibold">Automatic renewal</p><p className="mt-1 text-sm text-ink/55">{workspace.entitlement?.status === "non-renewing" ? "Renewal is off. Access ends on the date above." : "Paystack will renew this plan automatically."}</p></div><Button variant="secondary" disabled={Boolean(busyPlan)} onClick={() => manageSubscription(workspace.entitlement?.status === "non-renewing" ? "enable" : "disable")}>{workspace.entitlement?.status === "non-renewing" ? "Restore renewal" : "Cancel renewal"}</Button></CardContent></Card>}
