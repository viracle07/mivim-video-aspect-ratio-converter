...
export function BillingPanel() {
  ...
  const active = ["monthly", "yearly"].includes(workspace.plan) && workspace.billing?.status === "active";
  return <div className="space-y-6">
    <Card><CardContent className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-mist text-mivim-600">{active ? <CheckCircle2 className="h-5 w-5" /> : <CreditCard className="h-5 w-5" />}</div><div><p className="font-semibold">{active ? `${workspace.plan === "yearly" ? "Studio Yearly" : "Creator Monthly"} is active` : "Free uploads"}</p><p className="mt-1 text-sm text-ink/55">{active ? `Paid with Paystack · ${workspace.billing.currency || "NGN"}` : `${stats?.freeUploadsRemaining ?? 0} of 3 free uploads remaining`}</p></div></div>{active && <span className="w-fit rounded-full bg-mivim-600 px-3 py-1 text-xs font-medium text-white">Active</span>}</CardContent></Card>
